 ...
 * Supports both Nodemailer SMTP and Resend HTTPS API (for cloud platforms like Render)
...
async function sendOtpEmail(toEmail, otp) {
    ...
    // OPTION 1: Resend HTTPS API (Recommended for Cloud Hosting like Render)
    if (resendApiKey) {
        try {
            console.log(`[EMAIL] Attempting email send via Resend HTTPS API to ${toEmail}...`);
            ...
            if (response.ok) {
                console.log(`[EMAIL] OTP Email successfully sent via Resend API to ${toEmail}. ID: ${data.id}`);
                return { success: true, id: data.id };
            } else {
                console.error(`[EMAIL RESEND ERROR]`, data);
            }
        } catch (apiErr) {
            console.error(`[EMAIL API ERROR]`, apiErr.message);
        }
    }
    ...
    try {
        console.log(`[EMAIL] Attempting SMTP send via Nodemailer to ${toEmail}...`);

        // Use explicit SSL port 465 with timeouts for Render compatibility
        ...
        console.log(`[EMAIL] OTP Email successfully sent via SMTP to ${toEmail}. Message ID: ${info.messageId}`);
